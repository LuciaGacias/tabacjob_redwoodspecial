    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            local elements = {
              {label = 'Livraison', value = 'mule'},
              {label = 'Déplacement', value = 'gauntlet'}
            }

            if Config.EnablePlayerManagement and PlayerData.job ~= nil and
              (PlayerData.job.grade_name == 'boss') then
              table.insert(elements, {label = 'Déplacement Patronne', value = 'revolter'}) -- A mettre : https://gta.fandom.com/wiki/Redwood_Gauntlet
            end
